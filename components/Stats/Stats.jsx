        ...
        <div className="container relative z-2">
          ...
          <ul className="grid grid-cols-2 gap-3 sm:gap-4 md:gap-6 text-center md:flex md:items-center md:justify-between">
            {companyLogos.map((item) => (
              <li
                key={item.id}
                className="group relative overflow-hidden rounded-2xl sm:rounded-3xl border border-stroke-1 bg-n-1 px-3 sm:px-4 md:px-6 py-4 sm:py-6 md:py-8 transition-transform transition-shadow duration-300 hover:-translate-y-2 hover:shadow-[0_20px_60px_rgba(15,23,42,0.45)]"
              >
                ...
                <div className="relative z-10">
                  <p className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-semibold text-color-1 mb-1">
                    {item.value}
                  </p>
                  <p className="text-xs sm:text-sm md:text-base body-2 text-n-2">{item.label}</p>
                </div>
              </li>
            ))}
          </ul>
